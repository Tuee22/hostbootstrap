...
import HostBootstrap.Harness (
    CaseId,
    TestMatrix,
    TestMatrixError (InvalidVariantDeclaration),
    mkTestMatrix,
    mkVariantId,
    variantDraft,
    variantDraftId,
...
{- | One declared configuration variant of the demo's test matrix.

The name is the variant's reporting identity and the message is what the served
config carries, so a passing run proves the rendered page is config-driven: two
variants that differ only in this field must render two different pages. Both
are ordinary Dhall fields, which is what makes the variant set editable without
a Haskell change.
-}
...
selectable suites (the case ids plus @all@), the **test-config resource
overrides** projected into the test stack's config, and the declared variant
set. The file is generated by @test init@ and read by @test run@ (which builds
the run config from it).

@testVariants@ is what makes the matrix a projection of configuration rather
than of Haskell source (the worked-demo phase): adding a third variant is an
edit to the generated @\<project\>.test.dhall@, and every declaration is
validated before the run mutates anything.
-}
...
{- | The variant set @test init@ writes. Two variants that differ only in the
served message, so a passing run proves the page really is config-driven; an
operator adds a third by editing the generated file.
-}
demoDefaultTestVariants :: [TestVariantConfig]
demoDefaultTestVariants =
    [ TestVariantConfig{variantName = "hello-world", variantMessage = demoDefaultMessage}
    , TestVariantConfig{variantName = "hello-universe", variantMessage = "Hello, Universe!"}
    ]

defaultTestConfig :: Resources -> TestConfig
defaultTestConfig res =
    TestConfig{testResources = res, testVariants = demoDefaultTestVariants}
...
projectConfigFromContext
    ...
    cfgContext' =
        ProjectConfig
            ...
            , deploy = parentDeploy
            , message = parentMessage
            , -- The run's profile crosses every boundary with it, so a child frame
              -- resolves the same cluster plan the parent did.
              runProfile = parentRunProfile
            , webServiceConfig = parentWebServiceConfig
            , acceleratorServiceConfig = parentAcceleratorServiceConfig
            }
...
Every variant the run executes comes from @testVariants@, so adding, renaming, or
removing one is an edit to the generated @\<project\>.test.dhall@ rather than to
this module. Each declared name is validated into a 'VariantId' here, before the
matrix exists and therefore before the run mutates anything; an empty set is the
core's own 'EmptyVariantRegistry', and duplicate names are its
'DuplicateVariantIds'. Every case runs under every declared variant, which is
what makes the report card's row count the product of the two decoded sets.
-}
demoTestMatrix :: [CaseId] -> TestConfig -> Either TestMatrixError (TestMatrix Text)
demoTestMatrix caseIds tc = do
    drafts <- traverse declaredVariant (testVariants tc)
    mkTestMatrix
        caseIds
        drafts
        [(cid, map variantDraftId drafts) | cid <- caseIds]
  where
    declaredVariant declaration =
        case mkVariantId (variantName declaration) of
            Left invalid -> Left (InvalidVariantDeclaration invalid)
            Right ident -> Right (variantDraft ident (variantMessage declaration))
