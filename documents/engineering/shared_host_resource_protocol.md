...
> **Purpose**: Define the topology by which hostbootstrap and the other participating projects coordinate
> finite resources on one host without a shared scheduler, a new seed-to-seed dependency, or an early
> seed-to-amoebius dependency.
> **Read this if**: a hostbootstrap workload, resource-family extension, mechanism provider, project adapter,
> or substrate interpreter has to participate in shared-host admission.

This repository copy records only hostbootstrap metadata, navigation, and its proposed adoption boundary. It
does not establish implementation status or semantic release authority. The proposed protocol would be
Haskell-owned in a standalone neutral release and independently re-derived by amoebius.

## Current Status

This document is a target proposal. It establishes no neutral release, signed host catalog, implementation
status, or cross-project conformance claim. Current library relationships remain governed by the
[library hierarchy](../architecture/library_hierarchy.md).

All language below describes proposed behavior unless it explicitly says otherwise. Implementation and
qualification status remain owned by the
[Development Plan → Current Phase Status](../../DEVELOPMENT_PLAN/README.md#current-phase-status).
