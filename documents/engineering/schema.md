...
> **Purpose**: Define the single skeletal `hostbootstrap.dhall` the thin Python bootstrapper reads,
> and explain why the rich project and per-case test Dhall are generated by the project binary rather
> than declared here.

## TL;DR

- The repository-root `hostbootstrap.dhall` is **skeletal and identical in shape across projects**:
...
- The rich project-level Dhall (runtime roles plus cluster-bootstrap instructions) and the per-case
  ...
  they validate against. `hostbootstrap-core` owns only the skeletal-schema decoder.
...
## Why The Schema Is Skeletal

The skeletal schema deliberately carries **no** substrate, execution-model, lifecycle, or mount
...
> **WRONG** — declaring a substrate matrix or an execution model in the skeletal file
>
> ```dhall
> H.config
>   { project = "app"
...
1. **Skeletal `hostbootstrap.dhall`** (this document) — `project`, `dockerfile`, `resources` — read
