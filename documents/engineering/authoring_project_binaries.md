...
> **Purpose**: A step-by-step guide for a new consumer — author the project's `chain :: ProjectConfig ->
...
- **The chain is the project.** A consumer's primary contribution is a single ordered value,
  `chain :: ProjectConfig -> [Step]`, plus the actions for any project-specific `Step` kinds it adds. The
  core interprets that chain recursively under `project up`. The consumer extends the chain and the step
  vocabulary; the command surface (`ensure`, `context`, `project`, `test`, `check-code`) is fixed. The
...
1. **Define the root config and budget.** The project binary owns `<project>.dhall`: the resource budget,
   the Dockerfile path, the deploy settings, and the parameters the chain reads. `project init` writes the
   root `<project>.dhall` (a host-orchestrator config with no parent) with optional
   `--cpu/--memory/--storage/--ha-replicas`. Python derives the project name from the Cabal file and does
   not read Dhall. See [schema](schema.md) and [resource_budgeting](resource_budgeting.md).
2. **Author the chain.** Provide `chain :: ProjectConfig -> [Step]` — the ordered lift sequence that is the
...
deploy-chart    vm-project-container-2 (container) -- deploy the web service chart pod (NodePort 30080)
...
list, `deployChartStep` deploys the web pod, and `exposePortStep` verifies the endpoint. `project up` ends
at a live webservice on `localhost:30080`. The interpreter runs the steps in order and is restartable from
any frame.
...
`hostbootstrap-core` exposes exactly `ensure`, `context`, `project`, `test`, and `check-code`. The
consumer's contribution is `chain :: ProjectConfig -> [Step]` plus project step actions; the demo supplies
