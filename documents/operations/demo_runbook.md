...
- The demo's target config model uses sibling `hostbootstrap-demo.dhall` files: host, VM, container on the
...
`demo/.build/hostbootstrap-demo.dhall` and then edited by the user for project settings. The obsolete
`demo/hostbootstrap.dhall` file is removed; normal command gating reads the context section inside the
active sibling `hostbootstrap-demo.dhall`.
