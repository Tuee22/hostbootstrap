...
|--------|----------------|
| `HostBootstrap.HostTool` | Closed enumeration and absolute-path resolver for managed external tools (including `Python3`/`Flock`/`Lockf`, `Nvkind`/`NvidiaSmi`, and Windows `Winget`/`Nvcc`/`Wsl`). The prepared Incus route no longer resolves an interpreter or a locking front end: it resolves the provider client alone and holds its clauses through the protected store's exclusive entry. `Flock` and `Python3` survive at the cluster backend, and `Lockf` as the discriminator that refuses a host offering it in place of `flock`; the guest-alias route's own front ends are the guest's, reached through one absolute host-provider command. The [cluster-lifecycle, budgets, and cordoning phase](../../DEVELOPMENT_PLAN/phase-16-cluster-lifecycle-and-cordoning.md) narrows the enumeration when its drivers stop needing them. Most production paths consume resolved tools, but the repository-wide removal of residual bare-command calls is still open. |
