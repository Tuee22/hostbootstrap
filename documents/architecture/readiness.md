...
[`Readiness`](../../core/hostbootstrap-core/src/HostBootstrap/Readiness.hs). It is the signature
reference; this document describes its contracts without maintaining a second set of illustrative
signatures that can drift from the compiled API.

| Boundary | What it owns |
|---|---|
| `PollPolicy`, `PollPolicyError` | The opaque bounded retry policy. `mkPollPolicy :: Natural -> Micros -> Either PollPolicyError PollPolicy` refuses a zero or oversized attempt count and an overflowing total duration before polling begins; `withAttempts` re-derives one from an existing policy |
| `Micros`, `seconds`, `microsValue` | The one delay quantity, so a bare number is never a duration |
| `rolloutPoll`, `pushPoll`, `reachPoll`, `vmBootPoll`, `networkPoll`, `dockerPoll`, `nodePoll` | The named policies a caller selects instead of inventing constants |
| `ProbeResult a` | The total observation: `ProbeReady a`, `NotReady`, `Unavailable`, `ProbeConflicted ProbeConflict`, `Failed ProbeFailure`. Only `ProbeReady` is readiness |
| `ProbeConflict`, `ProbeFailure` | Structured conflict (expected, observed, remedy) and terminal failure (operation, cause) |
| `PollError`, `renderPollError` | `PollTimeout`, `PollUnavailable`, `PollConflict`, `PollFailed` — the exhausted-policy outcomes, each retaining its label and cause |
| `Decision a`, `pollStep`, `drivePure` | `Yield` / `Retry` / `GiveUp`, and the pure driver that makes the loop testable without effects |
| `pollUntilReady`, `pollUntilReadyWith` | The effectful loop over a `HostConfig -> IO (ProbeResult a)` probe |
| `ObservedReady`, `awaitObservedReady` | The non-authorizing compatibility observation, which mints no readiness evidence |
| `BackendProbeKey` | Plan- and resource-indexed readiness identity |

The plan- and operation-indexed types this readiness algebra feeds — `ResourceHandle`, `Ready`,
`Probe`, `DependencySnapshot`, `OperationPreconditionSet`, `PreparedPreconditions`,
`PreparedOperation`, and `OperationAdvance` — are owned by
[lifecycle_state_model](lifecycle_state_model.md#opaque-capabilities).
...
- `NotReady` is bounded and retryable. `Unavailable`, `ProbeConflicted`, and `Failed` remain distinct
  and retain operation, resource, and cause.
- `PollPolicy` is opaque. `mkPollPolicy` requires a positive, bounded attempt count and a `Micros` delay,
  computes the total duration with overflow-safe arithmetic, and rejects every value outside the
  documented maximum before polling begins.
- `Probe resource dependency` fixes the evidence kind a successful observation can mint; a caller cannot
  ...
  plan/backend modules after binding the concrete dependency relation. Only a managed dependency can enter the authorizing readiness path;
  probing an unmanaged or foreign dependency yields the non-authorizing `ObservedReady` instead.
- Every mutation declares its exact readiness requirements through a named transition or opaque
  operation descriptor minted by `ProjectPlan scope specDigest planId configId cfg`, constructed only with
  ...
  The plan first produces an opaque `DependencySnapshot` by internally traversing the
  ...
  verified journal record, and backend call definition to create `OperationPreconditionSet` and its
  matching verified backend call under fresh shared precondition-set and call-digest indices. Neither
  proof is a prerequisite for constructing the other; the plan-unique operation key already binds both
  back to the earlier descriptor. Its zero-dependency branch is private.
...
1. an external-package compile test cannot import the private `Ready` constructor, construct a `Probe`,
   or choose the successful probe's result tag — the standing guards are the `CallerForgedClusterProbe`,
   `ForgeReadyBlobRoute`, and `ObservedReadyGuestAlias` compile-fail fixtures;
