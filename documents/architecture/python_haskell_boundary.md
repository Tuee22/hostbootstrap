...
| Ensure the host **build** toolchain | Python | The prerequisites to build the binary host-native must exist first — on Apple, Homebrew → `ghcup` → GHC/Cabal; on Windows, **winget** (the Homebrew-analog pre-binary package manager) → the mingw32 GHC/Cabal toolchain plus MSVC; the equivalent on Linux. This is the metal frame's *provision* move. |
...
3. **(Provision)** Ensure the host build toolchain (on Apple, Homebrew → `ghcup` → GHC/Cabal; on
   Windows, **winget** → the mingw32 GHC/Cabal toolchain plus MSVC; the equivalent on Linux) — the
...
mingw32 GHC/Cabal toolchain plus MSVC, Python builds the native `hostbootstrap.exe` host-native, and
...
- **Windows (target).** The Windows metal frame — winget as the pre-binary package manager ensuring the
  mingw32 GHC/Cabal toolchain plus MSVC, the native `hostbootstrap.exe` built host-native, and the
  binary-owned `ensure wsl2` descent into the WSL2 Ubuntu-24.04 VM frame (the peer of Lima/Incus) — is
  the **target** shape, not yet implemented or hardware-validated. It is owned by the reopened phases
  (phase-2 substrate detection, phase-3 reconcilers, phase-11 VM provider). The ownership boundary is
  unchanged: winget + the toolchain + the `.exe` build are the only pre-binary Python work, while
  `ensure wsl2` and `ensure cudawin` are reconcilers the exe owns. See [wsl2](../engineering/wsl2.md).
