...
- The Python bootstrapper does only the **minimum to build the project binary**: derive the project
  ...
  host-native, then exec. The build-only Linux gate omits `/dev/kvm`; the `doctor` / `run` runtime gate
  keeps it because provider VM bring-up needs it. The sequence is **provision → build-pb → exec, with no config step** — Python
...
2. **(Provision)** Assert fail-fast host minimums (`build` uses the build-only floor; `run` uses the
   runtime/provider floor).
