...
The root's own `PreparedNodeGrant` is the same idea one level up, and it is built. It carries the canonical
signed response, the authorized node, that node's ordered dependencies, and the two gate packages — and no
store, session, record key, or compare-and-swap operation, so its evidence fold discloses what a frame may
...
more than was prepared. Only a `NextNode -> Prepared` answer becomes one: `Descend` and `Refused` are
...
| Sealed semantic lifecycle completion (§ W/§ X/§ Y/§ HH) | Cabal-private lower `Handoff.Completion` owns exactly five exports: the four-role-nominal `LifecycleCompletion`, three fixed-unit acknowledged producers, and its proof-only fold. Cabal-private upper `Handoff.Lifecycle` owns exactly two completed-report renderers and imports `Command.LifecycleEntry`; it does not import the lower owner. Both have zero production importers, Receiver imports neither, and no testing or Process/runtime seam exists | `ImportHandoffCompletion.hs`, `ImportHandoffLifecycle.hs`, `HandoffSpec`, `ProjectPlanSpec` |
...
| Rooted terminal receipt (§ W/§ X/§ Y/§ HH) — **target** | Only the root writes Published/Received: it publishes before signed `FrameComplete`, accepts idempotent `ReceiptConfirm`, and returns signed `ReceiptRecorded`. A child receives no `ProtectedStore`, raw receipt key, receipt mutation helper, or completion constructor | [the recursive-lifecycle-command phase](../../DEVELOPMENT_PLAN/phase-17-recursive-lifecycle-command.md) proof matrix |
