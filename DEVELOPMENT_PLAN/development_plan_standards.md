...
winget as the toolchain root plus the virtualization / WSL2 feature the nested WSL2 provider needs); **every other
host dependency the
...
- The Python bootstrapper ensures the host build toolchain (Homebrew → `ghcup` → GHC/Cabal on Apple; the
  equivalent on Linux; winget → GHC + MSVC on Windows) — the prerequisites to build the binary — then builds `./.build/<binary>`
  host-native and execs it. Python builds the host-native binary and execs it; it does not initialize or
  trigger config creation. The binary owns its Dhall — a normal command fails fast (exit 1) when no sibling
  `<project>.dhall` exists, and the config is created by an explicit `project init` or generated by the test
  harness (`psTestConfig`). A `./.build/<binary>` is always present on the host after the build.
...
The project binary also **emits its own schema** (`config schema`) and default config (`project init`),
reflected from its decoder types where possible so the schema cannot drift. Python derives the project
name from the Cabal file and has no Dhall-facing configuration role: Python builds the host-native binary
...
explicit `project init` or generated by the test harness (`psTestConfig`). Python never reads or writes
Dhall itself.
...
Python derives `<project>` from the Cabal file, builds the host-native binary, and execs it. Python builds
the host-native binary and execs it; it does not initialize or trigger config creation. The binary owns its
Dhall — a normal command fails fast (exit 1) when no sibling `<project>.dhall` exists, and the config is
created by an explicit `project init` or generated by the test harness (`psTestConfig`). The built binary
