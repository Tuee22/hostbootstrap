...
installed rather than treated as a manual prerequisite.** Each host dependency is represented by a
probe-first `ensure` reconciler — a host-applicability predicate plus a reconcile action — exposed to
...
Current implementation is weaker and must be described as such:

- demo `ProjectConfig` carries both refined top-level `resources` and a separately editable raw
  `context.resourceEnvelope`; validation does not require equality, lifecycle sizing reads the latter,
  and child-context projection copies that full envelope rather than the locally calculated cluster
  slice;
- the demo-local `clusterSliceOfBudget` is not a partition proof: below the full-lifecycle floor its
  `max`-based CPU/memory/storage floors can make a slice equal to or larger than its parent envelope.
  The normal root gate masks that case, but a separately supplied raw child envelope can still reach the
  function;
- top-level `Quantity`/`Resources`/replica/port/timeout decoding rejects selected malformed values, but
  constructors and `Num`/`IsString` paths remain public, the raw applied envelope bypasses those checks,
  and zero/bare-byte/below-provider-minimum quantities can reach later failure;
- `parseQuantity` is shared, but capacity checks use parsed bytes while Lima/Colima/Incus/WSL builders
  round memory/storage up to whole GiB; the provider-effective wall is not represented separately;
...
- new Lima/Incus/WSL resources receive initial sizing, but existing VM/VHDX sizing is not uniformly
  compared or reconciled. Direct Colima is unsized. Direct Linux GPU outer build/container effects are
...
Sprint 9.10 owns the exact admission/partition algebra, Sprint 9.4 the bare-Linux storage decision,
Sprints 5.7–5.8 and 11.10 the provider walls, Sprint 13.18 the complete demo workload projection, and
...
current project-owned `psServiceVariant :: cfg -> Either String String` projection attached with
`withServiceConfig` (§ AA). That arbitrary function can return a constant/unrelated key and is not a
validated relation. The target finalized spec jointly binds the hidden field row, full `ProjectCodec`,
role-wire `RoleCodec`, and typed registry. A parent transports only a role-specific descriptive wire;
child-local verification mints the opaque
...
streams sit alongside the project `check-code` action. The entrypoint validates those extension points before parser
...
the `ToDhall` encoder-declared full project-local `cfg` shape; and `project init` writes a default full
project config. The emitted expression comes from the encoder's `declared` value while `FromDhall`
supplies a separate `expected` expression; sharing a Haskell value type does not prove those expressions
equal.
...
lifecycle plan rather than a parallel `RunModel` selector or Dhall literal (§ T).
...
(threaded through a non-empty `TestSuite`); and the **service runtime seam** (currently an additive,
possibly empty `ServiceRegistry` plus arbitrary final-project `psServiceVariant`; target a finalized
typed registry and config/frame-indexed `SelectedService` package, § AA). A project integrates through a Cabal dependency
...
second `RunModel`/`selectRunModel` representation and not a `Core.dhall` field. Phase 10.10 removes those
...
`HostTarget = Local | InVM` is a definition-only predecessor and must not remain a parallel public
dispatch path; it is tracked in the cleanup ledger. L0 owns only the generic lift; the *specific* chain (the worked
...
independently supplied `psChain`, `psFrameContext`, and
`psTeardown` interpretations violate this doctrine and are removed by Phase 16.6. The current
`chain :: cfg -> [Step]` is the migration source and one forward ordering, not by itself the finished
...
chain, topology, teardown, lifecycle scope, or plan identity. The current `chain :: cfg -> [Step]` plus separate
`psFrameContext`/`psTeardown` fields is the migration source, not the finished single representation.
...
- `test init` — currently writes a per-project `<project>.test.dhall` containing a redundant
  `testSuites : List Text` plus resource overrides, without requiring a pre-existing sibling
  `<project>.dhall`. The target replaces that payload with typed case/variant selections and scoped
  config overrides such as resources or secrets. Its target request has no overwrite flag and uses
  `RefuseExisting`.
...
- `service schema` — currently prints the full encoder-declared `cfg` shape. The target prints the
...
`service run` is intended as a **leaf-frame runtime command, never an orchestrator**. Current core checks
the primary kind is `ClusterService` or `Daemon`, then calls arbitrary
`psServiceVariant :: cfg -> Either String String` and looks up its string in a possibly empty registry.
Core does not require a service field/ADT, relate selector output to config, or require a service-specific
capability; those are demo conventions. It also loads config for selection while the demo handlers reload
the sibling file, so dispatch is not an immutable snapshot.

The target removes the post-decode `cfg -> String` selector. The finalized project specification hides
the project-declared field row and jointly derives its full `ProjectCodec` and
`RoleCodec scope specDigest fields`; callers cannot pair codecs or choose `fields`. A validated parent config can
...
The generic `cfg`/`tcfg` substrate is implemented, but the current
`psTestConfig :: tcfg -> IO [(Text, cfg)]` can represent an empty list and stringly duplicate/invalid
labels. Generated-config cleanup is byte-conditional, and the demo still resolves its live cluster with
Production/`.data`. Phases 10, 13, 19, and 20 own those repairs; the target bullets below must not be read
as current isolation evidence.
...
- **No core defaults.** `defaultResources` / `defaultDeployConfig` / `defaultProjectConfig` and the
  `initAction` flag defaults are removed. Current `psInit`, `psTestInit`, and `psTestConfig` are
  independent project callbacks; the demo shares a helper between root/harness assembly only by
  convention, while its service projection still invents fallback ports/timeouts. The target
  `psAssemble` is the only default-bearing function and every role projection is total from its result.
...
- **Generic over the config type.** The extension contract is `ProjectSpec cfg tcfg`, parameterized over a
  project's config type `cfg` (its `<project>.dhall`) and test-config type `tcfg` (its
  `<project>.test.dhall`). The current `ProjectCfg` compatibility class requires both read-only
  `cfgContext` and production-unused raw `cfgWithContext`; the latter is not authority and must be
  deleted by Phase 19.7. The target couples through a read-only context projection, the scope-correct
  `ProjectCodec`/`ValidatedConfig` transition. The current command-specific
  `psServiceVariant :: cfg -> Either String String` is not retained as target authority: finalized
  project validation projects a role-specific wire; child-local `RoleCodec` verification jointly mints
...
- **`<project>.test.dhall` is a thin override and the harness generates the run's config (closes the § Z drift).**
  ...
  builds labeled config variants through the scope-aware restricted assembler (which can use declared
...
- **Generic secrets shape.** Core offers a pure `SecretRef = < Vault | TransitKey | Prompt | TestPlaintext >`
  vocabulary projects embed in `cfg`, so raw `Text` cannot occupy a secret-ref field. Core never resolves
  secrets; a project's `psTestConfig` may swap `Vault` pointers for `TestPlaintext` read from its own
  `test-secrets.dhall`. Because `TestPlaintext` is part of the same current union, excluding it from
  production remains a consumer code-check policy rather than a scoped core-type guarantee. The target
  is `SecretRef scope` plus `ProjectConfig scope`: the reflected Production schema has only
  ...
  schema decodes to an untrusted `HarnessConfigWire`. Grant and byte verification through the
  scope-correct opaque `ProjectCodec` jointly mint generic
  ...
  record update. Phase 19.7 owns this migration.
...
the image push, so must the durable-share mount gate the alias step (§ DD). The public
`HostBootstrap.Readiness.Internal` constructor is an open defect, not an acceptable test seam. Hiding
`MkReady` alone is insufficient while a caller can construct an always-ready `Probe` and choose an
independent result tag; the opaque plan/backend-produced `Probe resource dependency` fixes both.
...
### FF. Immutable Base Identity and Native Validation

Published base images are consumed by immutable identity. A mutable
`basecontainer-<flavor>-<arch>` tag is a discovery alias: build/publish records its resulting digest,
consumers pull and resolve the tag, and the derived build receives and verifies the digest-qualified
reference. Local unpublished bases never stand in for the published source of truth. A base input change is
complete only after the affected native-architecture tag is rebuilt, passed through the full Python and
Haskell quality gates, published, pulled, and used by a derived-image validation.

`base build` and `base build-and-push` validate that the requested architecture matches the native Docker
engine/host architecture before work starts. Cross-architecture emulation is not silently selected. Every
network-resolved tool, installer, and warm-store input is versioned or integrity-pinned; a successful
mutable lookup such as `latest`, an unversioned package install, or a live installer script is not
reproducibility evidence.

The host-native Cabal project and the Linux project-container Cabal project are distinct artifacts. The
host project never imports container-only `/opt/basecontainer/...` freeze paths; the container project
