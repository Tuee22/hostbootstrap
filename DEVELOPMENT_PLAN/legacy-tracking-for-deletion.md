...
owning phase. Both remaining entries are **retained by decision** (not slated for deletion): they are
tracked here for visibility — `prereqs.py` until its residual checks are lifted into
`hostbootstrap-core`, and `dhall_tool.py` permanently (the pre-binary skeletal config read). When a
surface is removed, its entry moves to **Completed** in the same change. Per
...
- **`python/hostbootstrap/dhall_tool.py`** — provisions and shells out to a pinned `dhall-to-json`
  ...
  decision (phase-6.1) means the pre-binary Python layer must decode the skeletal `hostbootstrap.dhall`
  itself (the cordon and container-build steps run before any project binary exists), so Python keeps a
  Dhall reader for the skeletal tier; the in-process Haskell `HostBootstrap.Config.Schema` decoder
  still owns the rich project/test tiers via the project binary. No longer slated for deletion.
## Completed

These surfaces were removed when the Python layer was shrunk to the thin five-step bootstrapper in
...
- **`python/hostbootstrap/models/*`** (`container.py`, `host_binary.py`, `host_daemon.py`,
  ...
  one binary through the build-twice / copy-out model (`python/hostbootstrap/bootstrap.py`), with
