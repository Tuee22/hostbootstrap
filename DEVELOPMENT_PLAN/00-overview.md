...
ensure the host toolchain, build the host-native binary and exec it. Python builds the host-native binary
...
The **Windows third-substrate** work reopens phases 2, 3, 6, 9, and 11 (`Done` → `Active`). Windows joins
`apple-silicon`, `linux-cpu`, and `linux-gpu` as the third metal substrate (`windows-cpu`/`windows-gpu`,
mingw32 toolchain) **woven into the existing phases — not a new phase**: substrate detection (phase 2), the
`ensure cudawin` / `ensure wsl2` reconcilers (phase 3 / phase 11), the thin Python bootstrapper building the
native `hostbootstrap.exe` host-native with winget as the Homebrew-analog pre-binary package manager
(phase 6), substrate-aware host-capacity (phase 9), and WSL2 (Ubuntu-24.04) as the Lima/Incus host-provider
VM peer (phase 11). In the same pass **Tart retires** — it was core-only and latent, so its prose is removed
here while the code deletion is a later phase-3 sprint — and composition pattern #7 **re-anchors to a
headless host build**, with `ensure cudawin` (CUDA-on-Windows) its first instance.
...
substrates are `apple-silicon`, `linux-cpu`, `linux-gpu`, `windows-cpu`, and `windows-gpu`. It is
`Active` (reopened): Windows joins as the third metal substrate (`windows-cpu`/`windows-gpu`), detected
alongside the Apple-silicon and Linux substrates.
...
steps. A wrong-host invocation fails fast with a one-line diagnostic. It is `Active` (reopened): the reconciler set is
docker, colima, lima, cuda, cudawin, homebrew, ghc, incus, wsl2 — `ensure cudawin` (CUDA-on-Windows headless
host build, the first instance of composition pattern #7) joins on `windows-gpu` and depends on phase-2
substrate detection, while `ensure tart` retires from the suite. The `ensure incus` reconciler is owned by
Phase 11.
...
pipx-installed wrapper itself. It is `Active` (reopened): on Windows the thin Python bootstrapper builds the
native `hostbootstrap.exe` host-native (the mingw32 peer of the macOS arm64 path), with winget the
Homebrew-analog pre-binary package manager.
...
It is `Active` (reopened): the substrate-aware host-capacity read extends to Windows
(`windows-cpu`/`windows-gpu`) alongside Apple silicon and Linux. The incus VM storage cordon is part of
Phase 11.
...
by the Apple Silicon demo path is implemented and validated through the full demo lifecycle. It is `Active`
(reopened): WSL2 (Ubuntu-24.04) joins as the Windows host-provider VM peer of Lima/Incus, depending on
phase-2 substrate detection and phase-6 Windows pre-binary bootstrap.
...
the global-architecture phases fan in on the inversion buildout and converge on the demo: │
  ...
  phase-11 (depends on 2, 3, 6, 9, 10)                                                     │
  ...
  Windows third-substrate (reopened, Done → Active): phase-3 `ensure cudawin` depends on 2 (substrate detection); phase-11 `ensure wsl2` host-provider depends on 2 (substrate detection) + 6 (Windows pre-binary bootstrap)
```
