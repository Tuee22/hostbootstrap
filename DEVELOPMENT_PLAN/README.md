...
| 10 | [Standardized test harness and execution shapes](phase-10-standardized-test-harness.md) | Active | Sprint 10.10 done; 10.9's project-wide mode/lease exclusion, fresh profile openers, abandoned-run sweep, and the recoverable run reservation that replaces the lock directory landed 2026-07-29, and the **four § EE clauses for the data root** and the **structured report-card outcomes** landed with it — the data root is the first § EE backend wired into a production route. The per-run `.test_data/<runId>` generation and the bound-Production recovery tranche (persisted/verified plan snapshots, the classified `bindRunLease`, both scope-exclusive recovery eliminators, and `RecoveredProductionLifecycleProfile`) landed 2026-07-29 at a clean 737/737 core gate. The two close transactions, the `ClosingProject` journal state, the complete-session proof, and the four-process reservation race landed 2026-07-30 at a clean 744/744 core gate — the race exposed and fixed a real defect: with no liveness established, a starting run swept a **live** run's lease and stole its mode, so two runs owned the project at once. **`verifyDestroySettled` landed 2026-07-30 with Sprint 16.6's teardown forest**, and `destroySettledClosure` converts it plus the complete-session proof into `SettledDestroyClose` closure evidence, closing that item. On 2026-08-01 the live harness path was narrowed to its exact owned store/project/root authority, snapshot persistence became immutable and byte-identically idempotent before lease binding, and every Session-owned multi-record transition moved behind the crash-consistent redo coordinator. On 2026-08-02 the first complete core-suite pass on Apple Silicon was recorded (**882/882**), after repairing two of this sprint's own gates that had stopped observing what they assert: the bound-snapshot assertion carried a second, stale copy of the snapshot wire format and now reads it back through the production decoder, and the compile-fail matcher compared GHC's line wrapping rather than its content. A live Apple Silicon reproduction on 2026-08-03 then **added an item and gave the known bound-recovery item a real failure to point at**: the generated sibling config was still claimed by a bare `<config>.hostbootstrap-test-owner` directory holding none of the four § EE clauses, and `test run`'s existence refusal fired *before* the abandoned-run sweep, so after an interrupted run the recovery machinery that did land was unreachable and both the config and the bound lease had to be cleared by hand. **Both halves closed 2026-08-04** at a clean 923/923 core gate: the new `Harness.GeneratedConfig` holds all four clauses over the file (origin record naming the intended payload digest, create-if-absent publication, bound kernel identity, identity-and-payload-conditional release) and shares one `Harness.Identity` layer with the data root so the two cannot drift; the three existence refusals are reconciled to the single post-sweep copy derived from installed project identity; and `classifyAbandonedBoundRun` gives the bound half a producer, so a bound abandoned run that `verifyNoProjectResourcesAcquired` proves acquired nothing is reclaimed and closed instead of only named. The same clean-tree gate exposed and fixed a pre-existing `-Werror` defect warm build trees had hidden (`HostTool`'s unconditional `(</>)` import). **Live-validated on Apple Silicon 2026-08-04 at `10/10 passed`** over four bring-ups and three destroys: the new config ownership acquired and released on every one, and the post-run audit found both leases `closed`, no `mode`/`config`/`dataroot` record left, `.test_data` empty with its parent preserved, and `demo/.data` intact — which also discharges the § C live re-run Sprint 16.6's plan-minted step descriptor owed on this lane. A follow-on landing the same day closed the deferred config-record settlement at a clean **924/924** core gate, and writing its test widened the defect: a run that reaches its release still holding the generated config left the record **and the file**, and because the lease closes on the way out no sweep could reach either — so the surviving file refused the next run with nothing able to resolve it. `releaseRun` now settles both owned objects before closing the lease, and a conflict is reported as its own `HarnessGeneratedConfigCleanupFailed` row. That change was **live-validated on Apple Silicon 2026-08-05** at `10/10 passed` from a pristine host (no `.data`, no `.build`, no Lima instance, no protected store) over four bring-ups and four destroys: both leases `closed`, no `mode`/`config`/`dataroot` record left, the generated sibling config gone, `.test_data` empty with its parent preserved, `.data` intact, and the Lima instance removed — so the new settlement ran on the real release path for both runs. The Linux CPU/GPU and Windows lanes still owe theirs. A second follow-on closed the § Z cross-profile race clause in-process at **925/925**, and it too found a real defect: the harness abandoned-run sweep read a **live Production invocation's** lease as an abandoned harness run and closed it, because `withProductionRoot` records its lease under the reserved run id `production` and the enumeration filtered on the lease-key prefix alone. The mode half was already guarded, so the operator-visible outcome was a refusal either way — the loss was the evidence Production's own bound recovery reads. The sweep now enumerates through `abandonedHarnessLeases`, and an open Production lease implies a held Production mode, so the harness is refused by the stated `ModeHeldByAnother` instead; that cross-profile branch is statically gated only, since the 2026-08-05 lane ran against a store with no Production lease. **The `withAbandonedHarnessRun` opener landed 2026-08-05** at a clean **930/930** core gate, giving the sweep's bound branch a real reopening: it jointly yields the abandoned run's durable snapshot, its already-bound lease, its own Harness mode, a `VerbDestroy`-only root, the `HarnessBoundRecovery` classification, and a `RecoveredHarnessClose` close root, all on a fresh broker generation — and `HarnessCloseRoot` gives harness close the root/verb half Production already had, with `currentHarnessCloseRoot` and `abandonedHarnessCloseRoot` as its only two producers and the live `test run` release path as its first production consumer. Writing it exposed two real defects in `closeHarnessRun`: it took the project and a bare `RunId` independently, so handed a different project of the **same config family** — which shares the `projectId` index — it closed nothing and returned `Right ()`, leaving the real run's lease and mode held behind a vacuous success; and it ignored its closure evidence entirely (the parameter was `_evidence`), so a settled destroy could take the short close and skip the Closing epoch that makes a mid-close crash resumable. Both were confirmed to reproduce against the pre-fix code. Open for the rest of that opener (the `AuthorityBroker`/`OldPermitFenceSet`/`VerifiedSessionOperationManifest` half and the recorded-session interpreter; the persisted-`Closing` and migration branches are now typed branches reached through the opener but still fail-closed; child-first teardown), for authenticated child admission and the live prepared/reconciler/terminal-recovery path, the receipt-carrying `Conflict`/`Unsupported` rows, and the remaining concurrency matrix |
