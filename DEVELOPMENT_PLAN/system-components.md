...
| `HostBootstrap.Command` | Partial | Parser/dispatch and command gates; exact `test`/`context` grammar plus command-specific missing-config recovery are the test-and-context-commands phase.4, and validated service dispatch is the service-runtime phase.6 |
