...
| `HostBootstrap.Harness.DataRoot.Native` | Implemented | The host identity backend behind that seam: POSIX `lstat` `(device, inode)`, and on Windows `GetFileInformationByHandle` over a handle opened with `FILE_FLAG_BACKUP_SEMANTICS` and `FILE_FLAG_OPEN_REPARSE_POINT`. Volume word first, little-endian, as the peer ownership backends encode it. The Windows branch has no native run yet |
...
| `HostBootstrap.Substrate.Provider` | Partial | Single provider launch/share/alias data route; direct-host aliases are removed. `Provider.Alias` supplies opaque prepared call/release and receipt-shaped definitions, but no provider-guest backend yet holds the four § EE ownership clauses, so production still uses the non-authorizing compatibility path. `spStop` also does not release the WSL2 wall, unlike its Lima/Incus peers |
...
| `HostBootstrap.Wsl2.GlobalWall` | Implemented pure foundation, not production authority | Exact present/absent origin, durable unknown phases, identity-bound stage/apply/restore classification, opaque receipt/authority values, and conflict-only recovery are modeled and focused-tested; generation/fence/tombstone integration and distinct live-versus-teardown authority remain open |
...
| `HostBootstrap.Wsl2.GlobalWall.Windows` / `cbits/wsl_global_wall.c` | Superseded, pending deletion | Bound-thread Win32 adapter and C shim written against the platform-primitive rule. The restated § EE clauses are satisfiable through `Win32`'s existing bindings (`getFileInformationByHandle` exposes the volume-serial/file-index pair), so the shim, its `c-sources`/`extra-libraries` block, the test-suite `-threaded` carve-out, and the `hb_wsl_*` foreign imports are tracked in [legacy-tracking-for-deletion.md](legacy-tracking-for-deletion.md). Replacement is one portable backend shared by every substrate |
