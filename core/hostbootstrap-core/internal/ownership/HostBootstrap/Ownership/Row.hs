...
import HostBootstrap.Ownership.Posix (posixObserveOwnershipManifest, posixOwnershipRow)
#endif
...
observeOwnershipManifestForHost :: Bool -> FilePath -> IO (Either OwnershipFault OwnershipManifest)
#if defined(mingw32_HOST_OS)
observeOwnershipManifestForHost _mutable _root =
    pure (Left (OwnershipUnsupported "the direct-Colima manifest row is unavailable on Windows"))
#else
observeOwnershipManifestForHost = posixObserveOwnershipManifest
#endif
