...
-- selectable suites (the case ids plus @all@, reflected from the project's
-- 'HostBootstrap.Harness.TestSuite' so it cannot drift) plus the **test-config
-- resource overrides** projected into the test stack's config. A consumer edits
-- @testResources@ to run its tests at a different budget than production; the
-- file is generated by @test init@ and read by @test run@.
...
-- override (seeded from the project config's resources by @test init@).
defaultTestConfig :: [Text] -> Resources -> TestConfig
defaultTestConfig suites res = TestConfig {testSuites = suites, testResources = res}

-- | The reflected Dhall type the @test.dhall@ decoder accepts (so the schema
...
-- | Write a @test.dhall@.
writeTestConfigFile :: FilePath -> TestConfig -> IO ()
writeTestConfigFile path cfg = TIO.writeFile path (renderTestConfig cfg <> "\n")
