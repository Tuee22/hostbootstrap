             ...
             in case persistedWallPhase active of
                    ...
                    WallOriginRecorded ->
                        let candidate = stageCandidate active
                         in case beginWallStageCreation
                                active
                                receipt
                                candidate of
                                Left err -> pure (Left (fromModelError err))
                    ...
                    WallApplied ->
                        withApplyLayout entry target active $ \layout ->
                            case settleWallApply
                                active
                                receipt
                                (applyObservation layout) of
                                Left err -> pure (Left (fromModelError err))
                                Right applied -> do
        ...
        case validateStageVolume active (wallObjectIdentity armedFile) of
            ...
            Right () ->
                case bindWallStage
                    ...
                    receipt
                    ObservedAbsent
                    (wallObjectIdentity armedFile) of
                    Left err -> pure (Left (fromModelError err))
                            ...
                            beginResult <-
                                withRestoreLayout entry target active $ \layout ->
                                    runWithAuthority active receipt $ \authority ->
                                        case beginWallRestore
                                            active
                                            authority
                                            receipt
                                            (restoreObservation layout) of
                                            Left err -> pure (Left (fromModelError err))
                                            Right restoring -> do
