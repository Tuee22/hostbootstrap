...
kindNodeCordonArgs clusterName = kindNodeCordonArgsFor (clusterName ++ "-control-plane")

-- | Apply a parsed envelope to one explicitly named kind/nvkind node container.
kindNodeCordonArgsFor :: String -> ResourceEnvelope -> Either String [String]
kindNodeCordonArgsFor containerName r = do
    b <- budgetFromResources r
    pure
        [ "update"
        , "--cpus"
