...
imports), so the admitted in-process decoder reads it back with no file or
network resolution. Representative render -> decode -> re-render tests cover
the project values that use this transformation.
-}
...
encoder's declared type, denoted so it compares structurally to the inline
occurrences inside an embedded value. The enclosing config's 'CodecWitness'
separately admits the complete decoder/encoder type pair.
-}
...
renderHoisted unions value =
    renderHoistedExpr unions (embed (Dhall.inject :: Encoder a) value)

{- | Hoist vocabulary unions in an already encoded expression. Validated codec
callers use this entrypoint so rendering does not independently select an
encoder instance.
-}
renderHoistedExpr :: [NamedUnion] -> Expr s Void -> Text
renderHoistedExpr unions value =
    Dhall.Core.pretty
        (hoistUnions unions (Dhall.Core.denote value))
