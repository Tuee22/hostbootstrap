...
-- @ServiceType@ ADT's constructor, e.g. @web@ / @workload-orchestrator@).
-- @service run \<variant\>@ dispatches on the variant; the registry may be
...
-- variant is the Dhall @ServiceType@ constructor name @service run@ dispatches
-- on; the action is the role body (a leaf-frame runtime, never an orchestrator,
