...
kindNameCases =
  ...
  where
    coreKinds =
        ...
        , DeployChart
        , ExposePort
        , PostHandoff "start-accelerator-daemon"
        ]
...
frameCases =
    ...
    , testCase "post-handoff hooks are not part of the pre-handoff segment" $ do
        map stepLabel (preHandoffStepsForFrame "host-orchestrator-0" demoChain)
            @?= ["launch the VM", "stage source into the VM", "ensure GHC in the VM", "build the binary in the VM"]
        map stepLabel (postHandoffStepsForFrame "host-orchestrator-0" demoChain)
